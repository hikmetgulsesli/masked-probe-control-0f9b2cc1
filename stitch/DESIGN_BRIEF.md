...
- Masked Probe Control turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a tiny single-page utility called Masked Probe Control. It should show three compact status tiles, one Refresh button that updates a timestamp and activity row, and one Ready/Paused toggle persisted in localStorage. Keep it simple and polished.
...
- Status Utility - Masked Probe Control

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Status Utility - Masked Probe Control
